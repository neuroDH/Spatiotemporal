...
for i=1:numel(stru_MergData_B)
    ...
    % Features %
    v_Feat = stru_MergFeat_B(i).PID_A;                                     % Change feature

    if s_Norm == 1
        v_Feat = (v_Feat-min(v_Feat))./(max(v_Feat)-min(v_Feat));
    end

    v_Axis = stru_MergData_B(i).PID_B;
    % v_Axis(1)=[]; % For IET
...
for i=1:numel(stru_MergData_S)
    ...
    % Features %
    v_Feat = stru_MergFeat_S(i).PID_A;                                     % Change feat
    if s_Norm == 1
        v_Feat = (v_Feat-min(v_Feat))./(max(v_Feat)-min(v_Feat));
    end
   
    v_Axis = stru_MergData_S(i).PID_B;
    % v_Axis(1)=[]; % For IET
...
for i=1:numel(stru_MergFeat_T)
    ...
    % Features %
    v_Feat = stru_MergFeat_T(i).PID_A;                                     % Change feat

    if s_Norm == 1
        v_Feat = (v_Feat-min(v_Feat))./(max(v_Feat)-min(v_Feat));
    end
   
    v_Axis = stru_MergData_T(i).PID_B;
    % v_Axis(1)=[]; % For IET
...
ylabel('mean amplitude','FontSize',s_Sz)
xlabel('pooled data in bins peer subperiods','FontSize',s_Sz)
title('LFP amplitude','FontSize',s_Sz)
