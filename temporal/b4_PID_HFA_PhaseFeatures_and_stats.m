...
%% Loading
load('PID_HFA.mat')
load('Quant_Phase_HFA.mat','stru_PhaseHFA')
%% Points pooled and bars (matrix for stats and charts)

for i=1:numel(stru_PhaseHFA)

    % Features %
    v_Feat = stru_PhaseHFA(i).Phase_IvsI;
    %v_Feat = (v_Feat-min(v_Feat))./(max(v_Feat)-min(v_Feat));
   
    v_Axis = stru_PhaseHFA(i).Phase_Time;
    %v_Axis(1)=[]; % For IET
...
ylabel('mean LFP vs HFA begin (s)','FontSize',s_Sz)
xlabel('pooled data in bins peer subperiods','FontSize',s_Sz)
title('LFP vs HFA begin','FontSize',s_Sz)
