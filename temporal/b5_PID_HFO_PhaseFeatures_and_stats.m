...
%% Loading
load ('HFO_BST.mat')
load('Quant_Phase_HFO.mat','stru_PhaseHFO')
%% Points pooled and bars (matrix for stats and charts)

for i=1:numel(stru_PhaseHFO)

    % Features %
    v_Feat = stru_PhaseHFO(i).Phase_IvsI;
    %v_Feat = (v_Feat-min(v_Feat))./(max(v_Feat)-min(v_Feat));
   
    v_Axis = stru_PhaseHFO(i).Phase_Time;
    %v_Axis(1)=[]; % For IET
...
ylabel('mean LFP vs HFO begin (s)','FontSize',s_Sz)
xlabel('pooled data in bins peer subperiods','FontSize',s_Sz)
title('LFP vs HFO begin','FontSize',s_Sz)
