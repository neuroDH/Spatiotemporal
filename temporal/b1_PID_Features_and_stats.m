...
for i=1:numel(stru_MergFeat)
    ...
    % Features %
    v_Feat = stru_MergFeat(i).PID_IEF;
    ...
    % str_Load = strcat(str_Data_Path,str_Slice,'.mat');
    % load(str_Load,'v_FilData')
    % v_IniAux = stru_MergData(i).PID_B;
    % v_EndAux = stru_MergData(i).PID_E;
    % 
    % v_PksAux = [];
    % 
    % for h=1: numel(v_IniAux)
    %     v_SegAux = v_FilData(v_IniAux(h):v_EndAux(h));
    %     [~,b] = max(v_SegAux);
    %     v_PksAux(h) = v_IniAux(h)+b;
    % end
    ...
    % plot(v_FilData)
    % hold on
    % plot(v_PksAux,v_FilData(v_PksAux),'*r')
    % xline([stru_MergData(i).SS,stru_MergData(i).TG],'m')
    % subplot(2,3,4)
    % plot(v_B_Ax,v_B_Ft,'*r')
    % subplot(2,3,5)
    % plot(v_S_Ax,v_S_Ft,'*r')
    % subplot(2,3,6)
    % plot(v_T_Ax,v_T_Ft,'*r')
...
%clearvars -except 'm_mean_B' 'stru_Data_B' 's_BinB' 's_BinS' 's_BinT'
%clearvars -except 'm_mean_S' 'stru_Data_S'
%clearvars -except 'm_mean_T' 'stru_Data_T'
%load('different_Slices.mat')
%save('different_Slices.mat')
...
figure('Position',[366.600000000000	64.6000000000000	848.800000000000	688.400000000000])
b = bar(v_Plot);
hold on
set(b, 'FaceAlpha', 0.2);
b.FaceColor = 'flat';
