...
%% Associate each PID with an HFA
...
for x=1:numel(cll_HFA)
    ...
    [v_B_Bag, v_S_Bag,v_T_Bag, v_B_A_Bag,v_S_A_Bag,v_T_A_Bag,v_BarPlot,v_Erro,stru_Joint] = ...
        f_Plot_Asociated(stru_MergData,stru_MergFeat,str_HFA,str_Feature);
...
%% Ratios
s_Bins = 23;
[m_BarHFA] = f_Count_HFA_Periods4(stru_MergData,s_Bins);
m_BarHFA(m_BarHFA==0)=NaN;

figure()
bar(mean(m_BarHFA,'omitnan'))

m_BarHFA(isnan(m_BarHFA))=0;
figure()
bar(mean(m_BarHFA),'k')
title(['Ratio num PID_{' 'HFA' '} / num PID']);

%% Ratios BST

[m_BarHFA] = f_Count_HFA_Periods3 (stru_MergData,[10,10,3]);

figure()
b = bar(mean(m_BarHFA),'k');
title(['Ratio num PID_{' 'HFA' '} / num PID']);
hold on
b.FaceColor = 'flat';
b.LineWidth = 0.8;
