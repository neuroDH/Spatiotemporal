...
load('Features2Plot_Uni.mat','stru_Win_PID')

%% Plot

s_NumFra = size(stru_Win_PID,2); 

%% PID amplitude

for i=1:s_NumFra
    v_Feat = stru_Win_PID(i).PIDAmp;
    v_Max(i) = max(v_Feat);
    v_Min(i) = min(v_Feat);
end
s_minData = min(v_Min);
